...
/* struct thread의 `magic` 멤버에 대한 임의의 값.
   스택 오버플로우를 감지하기 위해 사용됩니다. 자세한 내용은 thread.h 상단의 큰
   주석을 참조하세요. */
...
/* THREAD_READY 상태인 프로세스 목록, 즉 실행 준비가 되었으나 실제로 실행 중이지
 * 않은 프로세스들입니다. */
 ...
 * CPU의 스택 포인터 `rsp`를 읽고, 페이지 시작 부분으로 반올림합니다.
   `struct thread`는 항상 페이지의 시작 부분에 있고 스택 포인터는 중간 어딘가에
 있으므로, 이를 통해 현재 스레드를 찾을 수 있습니다. */
...
/* 스레딩 시스템을 초기화하여 현재 실행 중인 코드를 스레드로 변환합니다.
   이것은 일반적으로 작동하지 않으며, loader.S가 스택의 하단을 페이지 경계에
   놓았기 때문에 이 경우에만 가능합니다.
...
/* NAME 이름과 주어진 초기 PRIORITY를 가진 새로운 커널 스레드를 생성하고,
   ...
   있습니다. 반대로, 원래 스레드는 새 스레드가 스케줄될 때까지 얼마든지 실행될
   수 있습니다. 순서를 보장하려면 세마포어 또는 동기화의 다른 형태를 사용하세요.
...
tid_t thread_create(const char *name, int priority, thread_func *function,
    ...
    struct list child_list_of_parent = t->parent->child_list;
    /*file descriptor*/
    t->fdt = palloc_get_page(PAL_ZERO);
    t->next_fd = 2;
    // 주의! thread가 죽으면 palloc_free_page(t->fdt); 진행하기.
...
/* 차단된 스레드 T를 실행 준비 상태로 전환합니다.
   이것은 T가 차단되지 않은 경우에는 오류입니다. (실행 중인 스레드를 준비 상태로
...
bool cmp_wake_up_time(const struct list_elem *a, const struct list_elem *b,
                      void *aux) {
    struct thread *thread_a = list_entry(a, struct thread, elem);
    struct thread *thread_b = list_entry(b, struct thread, elem);
...
struct thread *thread_current(void) {
    ...
    /* T가 실제로 스레드인지 확인합니다.
       이 중 어느 하나라도 주장이 실패하면, 스레드의 스택이 오버플로우 되었을 수
       있습니다. 각 스레드는 4 kB 미만의 스택을 가지고 있으므로, 몇 개의 큰 자동
       배열이나 적당한 재귀 호출이 스택 오버플로우를 일으킬 수 있습니다. */
    ASSERT(is_thread(t));
...
/* 현재 스레드를 스케줄에서 제거하고 파괴합니다. 호출자에게 절대 반환되지
 * 않습니다. */
...
/* CPU를 양보합니다. 현재 스레드는 대기 상태로 만들어지지 않으며, 스케줄러의
 * 재량에 따라 즉시 다시 스케줄될 수 있습니다. */
...
void thread_sleep(int64_t ticks) {
    ...
    if (curr != idle_thread) {
        ...
        list_insert_ordered(&sleep_list, &curr->elem, cmp_wake_up_time,
                            NULL);  // for priority...
    ...
    thread_block();  // thread_current의 status를 BLOCKED로, schedule()진행.
                     // 순서 굉장히 중요.
    intr_set_level(old_level);
}

void thread_wakeup(int64_t ticks) {
    if (ticks < min_wake_ticks) return;
    ...
    for (e; e != list_end(&sleep_list);) {
        ...
        if (t->wake_up_time <= ticks) {
            e = list_remove(e);
            thread_unblock(
                t);  // status를 READY로 바꾸고, ready_list에 push까지 하는함수.
...
void thread_set_priority(int new_priority) {
    ...
    donate_priority();
    restore_priority();  // donate_priority()와 restore_priority()순서는
                         // 중요하지 않음. 결과가 똑같음.
    thread_switching();
}
...
int thread_get_load_avg(void) {
    enum intr_level old_level = intr_disable();
    int load_avg_100 = fp_to_int_round(load_avg * 100);
    intr_set_level(old_level);
    return load_avg_100;
}
...
void calculate_priority_mlfqs(struct thread *t, void *aux UNUSED) {
    ...
    int new_priority =
        PRI_MAX - fp_to_int_round(div_mixed(t->recent_cpu, 4)) - t->nice * 2;
...
void calculate_recent_cpu(struct thread *t) {
    if (t == idle_thread) return;
    int new_recent_cpu =
        div_fp(mult_mixed(load_avg, 2), add_mixed(mult_mixed(load_avg, 2), 1));
...
static void idle(void *idle_started_ UNUSED) {
    ...
    for (;;) {
        ...
        /* 인터럽트를 다시 활성화하고 다음 인터럽트를 기다립니다.
           ...
           따라서 이 두 명령어는 원자적으로 실행됩니다. 이러한 원자성은
           중요합니다; 그렇지 않으면 인터럽트를 다시 활성화하고 다음 인터럽트가
           발생하기를 기다리는 사이에 인터럽트가 처리될 수 있으며, 이로 인해
           최대 한 클록 틱의 시간을 낭비할 수 있습니다.

           [IA32-v2a] "HLT", [IA32-v2b] "STI", 및 [IA32-v3a] 7.11.1 "HLT
           명령어"를 참조하십시오. */
...
/* 스케줄할 다음 스레드를 선택하고 반환합니다. 실행 대기 큐에서 스레드를
   반환해야 합니다. 실행 대기 큐가 비어있으면 idle_thread를 반환합니다. */
static struct thread *next_thread_to_run(void) {
    if (list_empty(&ready_list))
        return idle_thread;
    else
...
struct list_elem *get_list_head() { return &ready_list.head; }
...
static void thread_launch(struct thread *th) {
     ...
     * 주의할 점은, 전환 작업이 완료될 때까지 여기서 스택을 사용해서는 안
     * 됩니다. */
    __asm __volatile(
        /* 사용될 레지스터들을 저장합니다. */
        "push %%rax\n"
 ...
 * 전환합니다. schedule()에서 printf()를 호출하는 것은 안전하지 않습니다. */
static void do_schedule(int status) {
    ASSERT(intr_get_level() == INTR_OFF);
...
static void schedule(void) {
    ...
    if (curr != next) {
        /* 우리가 전환하는 스레드가 죽어가고 있다면, 그 스레드의 구조체를
           파괴합니다. 이 작업은 thread_exit()가 자신을 종료하는 것을 방지하기
           위해 늦게 일어나야 합니다. 현재 스택에서 페이지가 사용되고 있으므로
           여기서 페이지 해제 요청을 큐에 넣습니다. 실제 파괴 로직은
           schedule()의 시작 부분에서 호출됩니다. */
