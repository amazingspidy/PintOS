...
duplicate_pte(uint64_t *pte, void *va, void *aux) {
    ...
    /* 5. 주소 VA에 WRITABLE 권한으로 자식의 페이지 테이블에 새 페이지를 추가합니다. */
...
__do_fork(void *aux) {
    ...
    /* TODO: 여기에 코드가 들어갑니다.
     * TODO: 힌트) 파일 객체를 복제하려면 include/filesys/file.h의 `file_duplicate`을 사용하세요.
...
/* PHDR가 파일 내에서 유효하고 로드 가능한 세그먼트를 설명하는지 확인하고, 그렇다면 true를 반환 */
static bool
validate_segment(const struct Phdr *phdr, struct file *file) {
    ...
    /* 페이지 0 매핑 금지.
       페이지 0을 매핑하는 것은 나쁜 생각이며, 허용한다면 사용자 코드가 시스템 호출에 null 포인터를 전달하여 memcpy() 등에서 null 포인터 단언문을 통해 커널을 패닉시킬 수 있음 */
 ...
 * - UPAGE에서 READ_BYTES 바이트는 OFS 오프셋에서 시작하는 파일에서 읽어야 합니다.
 * - UPAGE + READ_BYTES에서 ZERO_BYTES 바이트는 0으로 초기화되어야 합니다.
 * 이 함수에 의해 초기화된 페이지는 WRITABLE이 true이면 사용자 프로세스가 쓰기 가능해야 하며, 그렇지 않으면 읽기 전용이어야 합니다.
 ...
 * KPAGE는 palloc_get_page()로 얻은 사용자 풀의 페이지여야 합니다.
